...
- Huérfanos en disco detectados (no son bugs, son trabajo en progreso de otra sesión, no tocar sin confirmar con Juan): `modules/bibles/rv-verbo-strong-candidate/`, `rv-verbo-strong-nt-open/`, `rv-verbo-strong-pilot/` (variantes de Strong en evaluación), `modules/commentaries/barnes/` (comentario nuevo aún no integrado a `registry.json`), `modules/commentaries/matthew-henry-es/` (traducción manual abandonada, ver "Estrategia cambiada 2026-06-28" arriba — candidato a borrar más adelante si se confirma que ya no se necesita).
